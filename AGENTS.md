...
- **使用情况页的四条聚合必须走覆盖索引。** `request_logs` 实测单库 2.9GB（3930 行 × 平均 517KB 请求体 / 57KB 响应体），body 溢出页主导这张表。`UsageByProvider/ByModel` 走 `usage_events` 的 `idx_usage_events_provider/model`；`UsageByKey/ByCredential` 走 `request_logs` 的 `idx_request_logs_token/credential`。两条硬约束：一是 `idx_request_logs_credential` 的 WHERE 必须写成可索引形式（`credential_id <> '' OR credential_name <> ''`），`COALESCE(credential_id,credential_name) <> ''` 会让 SQLite 放弃覆盖索引、裸扫全表（实测同一查询 0.4s vs 2ms，打开面板即抖出几百毫秒磁盘 IO）；二是这些索引只能建在补列迁移**之后**——老库重建前没有 `credential_id`，写进内联 DDL 会在建表后立刻执行而报 `no such column`。防止退化的回归测试是 `TestUsageBreakdownAvoidsFullTableScan`（断言四条查询计划都含 `COVERING INDEX`）。首次升级会给存量库一次性建这三个索引，2.9GB 库实测约 8–11s（纯 IO 密集，与 page cache 大小无关，实测 2MB/8MB/64MB 同档），此后不再重复。
- **请求日志页的过滤组合会退化成裸扫，因此前端 loading 是必需而非装饰。** 无过滤与单条件查询都命中索引（`ORDER BY created_at DESC,id DESC LIMIT 15` 走 `COVERING INDEX idx_request_logs_created_at`，默认统计走 `idx_request_logs_filter`，实测 0–30ms）；但 `success = ?` 叠加 `LIKE '%…%'` 文本过滤时执行计划是裸 `SCAN request_logs`（`idx_request_logs_filter` 以 `token_id` 为前导列，`provider_name/client_model` 上不存在的 `success` 值配合无谓词约束的 LIKE 无法收敛），2.9GB 库上把 body 溢出页全读一遍。实测 0.35s（热）/ 8.5s（冷页缓存，注意冷值不可复现，热值是真实稳态）。因此 `RequestLogs` 的 loading 不是可选体验优化，而要有 **180ms 延迟门帘**（`LOG_SLOW_MS`）：正常查询全程无遮罩、不闪烁，退化查询与冷启动才点亮。列表查询与详情查询（`getRequestLog` 拉完整 body，单条可达数百 KB）各有一个 `useRef` 序号守卫，因为查询条件变化或连续点两行时旧请求可能后返回，必须只让最新一次落状态；详情弹窗关闭时同样作废在途请求，否则迟到响应会把已关的弹窗重新打开。
